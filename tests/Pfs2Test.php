...
class Pfs2Test extends PHPUnit_Framework_TestCase
{
    ...
    public function testMimeTypeAndOsCriteriaShouldYieldCorrectPlugins()
    {
        ...
        sort($expected); sort($results['foobar-media']['aliases']);
        $this->assertEquals($expected, $results['foobar-media']['aliases']);

        // Ensure the expected versions are sent
        $this->assertTrue( !empty($results['foobar-media']['releases']['99.9.9']),
            'Release v99.9.9 of "foobar-media" should be returned');
        $this->assertTrue( !empty($results['foobar-media']['releases']['100.2.6']),
        ...
        foreach (array('99.9.9', '100.2.6') as $version) {
            $this->assertEquals(
                'Foobar Media Viewer', 
                $results['foobar-media']['releases'][$version]['name'],
        ...
        $this->assertEquals('foobar-win-100.2.6', 
            $results['foobar-media']['releases']['100.2.6']['guid']);
        $this->assertEquals('latest', 
            $results['foobar-media']['releases']['100.2.6']['status']);
        $this->assertEquals('foobar-bad-99.9.9', 
            $results['foobar-media']['releases']['99.9.9']['guid']);
        $this->assertEquals('vulnerable', 
            $results['foobar-media']['releases']['99.9.9']['status']);
    ...
    public function testLaterUpdatesShouldWork()
    {
        ...
        $this->assertEquals('latest', 
            $results['foobar-media']['releases']['200.9.9']['status']);
